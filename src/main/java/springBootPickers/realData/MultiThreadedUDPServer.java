...
import springBootPickers.domain.StockA3;
...
public class MultiThreadedUDPServer {
	...
	private static final Logger log = LoggerFactory.getLogger(MultiThreadedUDPServer.class);
	   ...
	   private static final MultiThreadedUDPServer instance = new MultiThreadedUDPServer();
	    ...
	    public void startServer() {
	        ...
	        try {
	            ...
	            log.info("MultiThreaded UDP Echo Server is listening on port 22101");
	        } catch (IOException e) {
	            log.error("UDP 메시지 처리 중 오류", e);
	        }
	    }
	    private static class UDPHandler extends IoHandlerAdapter {
	        ...
	        public void exceptionCaught(IoSession session, Throwable cause) throws Exception {
	            log.error("UDP Handler 예외 발생", cause);
	        }
	        @Override
	        ...
	        public void run() {
	            try {
	                ...
	                if (buffer.remaining() < 195) {
	                    ...
	                    log.debug("T_id ={}", threadId + " : " + bufferContent);
	                    return;
	                }
	                byte[] data = new byte[buffer.remaining()]; // 남은 데이터 크기만큼 배열 생성
	                buffer.get(data); // 버퍼 내용을 배열에 복사
	                //log.debug("Buffer contents: {}", new String(data, StandardCharsets.UTF_8));
